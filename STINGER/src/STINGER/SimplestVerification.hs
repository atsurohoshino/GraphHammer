-- |STINGER.SimplestVerification
--
-- We will try to verify correctness of different ordering of operations.
...
module STINGER.SimplestVerification(
	  Index	-- from G500.
	-- HList
	, Nil
	, (:.)
...
data STINGER as = STINGER {
	  stingerEdges		:: Map.Map Index (Set.Set Index)
	-- Results of analyses.
...
stingerNew maxVertices = do
	return $ STINGER Map.empty Map.empty Set.empty

stingerEdgeExists :: Index -> Index -> STINGERM as Bool
stingerEdgeExists start end
	| start == end = return True
	| otherwise = do
	liftM (Set.member end . Map.findWithDefault Set.empty start. stingerEdges) get
...
stingerInsertEdge start end
	...
	| otherwise = do
		...
		modify $ \st -> st {
			  stingerEdges = Map.insertWith Set.union start (Set.singleton end) $
				stingerEdges st
...
stingerGetEdges start =
	liftM (Set.toList . Map.findWithDefault Set.empty start . stingerEdges) get
